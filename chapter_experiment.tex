...
\label{chapter:experiment:testbed}
    ...
    \label{sec:peanut}
        ...
        \label{sub:enoslib}
            ...
            A recent experiment engine, called \texttt{EnOSlib}~\cite{enoslib}, appears to be promising. It is a Python
            library that allows an easy deployment and execution of experiments. The experiment scripts written with
            \texttt{EnOSlib} are concise and readable, thanks to well designed abstractions. Compared to \texttt{Peanut}
            which has been solely implemented and used for Grid'5000 platform, \texttt{EnOSlib} offers multiple
            backends.  One issue is that it currently does not support the archive building of \texttt{Peanut} with the
            automatic collection of experiment data and meta-data.

            At the time we started working on \texttt{Peanut}, we did not know about \texttt{EnOSlib}, which was at its
            early stage of development and not yet advertised. Today, we believe it would be better to join forces and
            port the main features of \texttt{Peanut} in \texttt{EnOSlib}.
...
\label{chapter:experiment:difficulties}
        ...
        \label{sub:parameter_space:dgemm}
            ...
            \begin{itemize}
                \item Instead of sampling the product uniformly with \(MNK\sim\unif{1}{S}\), we made the choice to use a
                    use a quasi-random approach. We first compute $\gamma$ different values that are uniformly
    ...
    \label{sec:randomizing_order}
        ...
        thereby threatening the analysis experiment. This is confirmed by
    ...
    \label{sec:beware_of_extrapolations}
        ...
        a warning whenever a model is used too often outside of its parameter space.
...
\label{chapter:experiment:tests}
    ...
    \label{sec:test:implementation}
        ...
        \label{sub:workflow}
            ...
            \begin{figure}[htpb]
                \centering
                \includegraphics[height=0.74\textheight]{img/experiment/non_regression/implementation/workflow.pdf}
            ...
            The first one is implemented with \texttt{peanut} while the two others are implemented with \texttt{cashew}.
            \begin{description}
