...
\label{chapter:experiment:randomizing}
    ...
    \label{sec:parameter_space}
        ...
        \texttt{MPI\_Recv} or \texttt{MPI\_Send}, their duration is proportional to $S$, the size of the buffer that is
        being communicated, and (2) calls to the \texttt{dgemm} function, whose duration is proportional to the product
        ...
        \subsection{MPI communications}%
            ...
            spread, but rather exponentially. For instance, the \texttt{MPI\_Send} on the dahu cluster has four
            ...
            \NSI{1}{\mibi\byte} L2 cache. Then, the 16 nodes of a same CPU share a \NSI{22}{\mibi\byte} L3 cache and
        ...
        \subsection{Function \texttt{dgemm}}%
    ...
    \label{sec:randomizing_order}
        ...
        concatenation. The observed durations for function \texttt{MPI\_Recv} with both methods are presented in
        ...
        \begin{figure}[htpb]
            ...
            \caption{The duration of \texttt{MPI\_Recv} is piecewise linear, with several modes for small messages.}%
        ...
        \begin{figure}[htpb]
            ...
            \caption{The average durations of \texttt{MPI\_Recv} in the non-shuffled case still show several modes,
        ...
        \NSI{900}{\byte} and \NSI{1000}{\byte}. Each point is the duration of an individual call to \texttt{MPI\_Recv},
        ...
        In the non-shuffled case, on the left, the durations of two message sizes are different, namely \NSI{960}{\byte}
        and \NSI{999}{\byte}. For these two calls, the distributions have only one mode, so their average durations are
        significantly shifted. The assumption of identical distribution for these different message sizes is clearly not
        satisfied here.

        \begin{figure}[htpb]
            \centering
            \includegraphics[width=0.8\linewidth]{img/experiment/randomizing_order/distribution.png}
            \caption{The duration of \texttt{MPI\_Recv} is not identically distributed in the non-shuffled case.
            Durations truncated to \NSI{4}{\micro\second} for a better readability.}%
            \label{fig:randomizing_order:distribution}
        \end{figure}

        The root cause of this phenomenon is still unclear. Since shuffling correctly the experiments prevents the
        occurence of this issue, a possible reason may be that the individual calls to \texttt{MPI\_Recv} are not truly
        independent. The sequence before the calls for the sizes like \NSI{960}{\byte} or \NSI{999}{\byte} would lead
        to particularly good conditions and thus an excellent performance, which does not systematically happen in the
        shuffled case because of the proper randomization.
        ...
        measures and vice-versa. This change however did not bring any noticeable effect on our observations.
