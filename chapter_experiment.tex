    ...
    \section{State of the art}%
        ...
        \label{sub:existing_work}
            ...
            \subsubsection{Benchmarking software}%
                ...
                implementation of \texttt{spotrf}, which computes a Cholesky factorization of a given matrix. The purple
                line is the observed performance in reality, the green line is the estimation made in simulation. The
                large jump on 01/01/2018 is due to a hardware upgrade. The real runs exhibit an extremely large
                variability, which is mainly due to platform unstability. Since the StarPU team is interested in
                detecting regressions in StarPU itself and not the platform, these predictions are extremely valuable to
                them due to their reliability, they allow to rule out the false conclusions that would be made by
                looking only at the real runs.
                \begin{figure}[htpb]
                    \centering
                    \includegraphics[width=0.8\linewidth]{img/experiment/non_regression/state_of_art/starpu.png}
                ...
                We would like to stress that the vast majority of benchmarking tools we know of only report a
                performance metric (\eg a duration, a network latency, a number of page faults, etc.), they do not
