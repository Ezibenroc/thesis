...
\label{chapter:experiment:testbed}
    ...
    \label{sec:state_of_the_art}
        ...
        \label{sub:grid_5000}
            ...
            Nearly all the experiments presented in this document have been carried out on the Grid'5000~\cite{grid5000}
        ...
        \label{sub:experiment_engines}
            ...
            Automating the experiment is a necessary condition for reproducible results. A first step toward this
            ...
            typical \execo experiment uses many low-level constructs that are really cumbersome and unintuitive to write
            and read. Section~\ref{sub:comparison_with_execo} presents a comparison between the approach we implemented
            and \execo. Furthermore, \execo lacks
    ...
    \label{sec:peanut}
        ...
        \label{sub:comparison_with_execo}
            ...
            In this section, we use a small example to illustrate key differences between \peanut and
            \execo. The goal is to write an experiment that will take several nodes on a given Grid'5000
            ...
            First, Listing~\ref{lst:engine:execo} shows such an experiment using \execo. It should be executed as: \verb#python script.py#\\

            \lstinputlisting[language={Python},label={lst:engine:execo},
                caption={Small experiment example using \execo.}]{
                img/experiment/engine/mweExeco.py
            }

            \pagebreak
            This script, albeit fairly small, is already difficult to read in some places. For instance, in lines 11-12,
            ...
            It should be executed as: \verb#peanut script.py run tocornebize \# \\
            \verb#    --deploy debian9-x64-base --cluster dahu --nbnodes 2 \# \\
            \verb#    --walltime 00:20:00 #
            ...
            The script is not only half as long, it is also arguably much easier to read. Furthermore, it
            accomplishes a lot more than Listing~\ref{lst:engine:execo}, as it produces a peanut archive that contains
            all the metadata related to the experiment.

        \subsection{\enoslib, a promising alternative}%
        \label{sub:enoslib}
            ...
            implemented and used for the Grid'5000 platform, \enoslib offers multiple backends.  One issue is that it
...
\label{chapter:experiment:difficulties}
    %% TODO
    %% Calibration experiments: randomization. pour quoi faire ?
    %% 1. éviter les biais (randomization de l'espace de paramètres)
    ...
    \begin{itemize}
        ...
        \item The experiment order (in this example, the order of the sizes). The rationale here is to avoid temporal
    ...
    \label{sec:randomizing_order}
        ...
        calls with sizes \NSI{705}{\byte}, \NSI{741}{\byte} and \NSI{782}{\byte} (in orange) have durations above
    ...
    \label{sec:randomizing_sizes}
        ...
        \label{sub:effect_experiment_file}
            ...
            demonstrate this effect, we have generated three different experiment files using exactly the same generation
            method described previously. These three experiments, named \texttt{A}, \texttt{B} and \texttt{C}, were
            repeated several dozens of times during a weekend in a random order. They have been carried on 8 different
            ...
            are for a tall and skinny matrix, with \(K\geq3000\), which corroborates the hypothesis of a bad
            cache utilization.
        ...
        \label{sub:fixed_size}
            ...
            performed one experiment with each of these files in a random order during a weekend on two nodes of the
            ...
            with the five experiments. We can observe that again, the generation method for the size sequence has an
            enormous effect on the experiment. First, using a fixed value for \(K\) reduces very significantly the
            inter-run performance variability. The average performance is also greatly affected, it is the highest with
            \(K\) fixed to 128, the lowest with \(K\) fixed to 256 or 512 or with the random generation, and it is
            ...
            Figure~\ref{fig:randomizing_sizes:expfile:fixing_K:power_CPU}, is particularly notable, and peculiar. The random
            ...
            performance. This will affect any statistical model that relies on the measured data, not because of a
    ...
    \label{sec:randomizing_data}
        ...
        for our simulations. Our predictions were wrong, so we investigated further and we noticed a significant mismatch
        ...
        experiments described in this section were done with matrices of fixed size \(2048\times2048\), but the
        ...
        \label{sub:randomization_matrix_initialization}
            ...
            \begin{enumerate}
                ...
                \item The elements of the matrices are an increasing sequence in the interval \([0, 1]\). More
        ...
        \subsection{Hypotheses}
            ...
            matrices will have more bits in common, this would cause fewer bit flips than the random initialization but
        ...
        \subsection{Conclusion}
            ...
            To respect its energy budget, the CPU has to throttle more aggressively its frequency when matrix elements
            are more diverse and thus more energy consuming.
        ...
        \subsection{Related work}%
            M. La{\ss}, C. Plessl and R. Schade (Paderborn Center for Parallel Computing) independently made similar
            ...
            \begin{itemize}
                \item They observed that \dgemm performance depended on the content of the matrix. They also
                    ...
                    argument in favor of this hypothesis. Their experiment was done using another \dgemm
                    implementation than the one used in this work (Intel MKL) on an Intel Xeon Gold 6148 (Skylake-SP
                    family) from a noctua
    ...
    \label{sec:beware_of_extrapolations}
        ...
        On several occasions when working on Part~\ref{part:prediction}, we found that our predictions were inaccurate
    ...
    \label{sec:beware_of_experimental_conditions}
        ...
        The two last scenarios may seem contrived. The goal was, again, to implement a micro-benchmark that is as
	...
	\section{Conclusion}
        ...
        %% - La randomization est très importante pour se prémunir de biais expérimentaux. Or, des fois on cherche
        %%   justement ces biais.
        ...
        were only revealed through randomization (or the lack thereof). However, in some cases, we needed our
        ...
        experiments (\eg cooling issues, BIOS upgrades). With this kind of experimental artifacts, it can become more difficult to
        ...
        automate the entire experimental process (thereby greatly reducing the human errors) and collect relevant
        information and (2) making regular tests on the platform to detect possible changes.
...
\label{chapter:experiment:tests}
    ...
    performance across different time periods. The reasons are multiple, ranging from the hardware to the software. This
    motivated the implementation of performance non-regression tests, to help us detect any noteworthy change on the
    platform. In this chapter, we start by giving an overview of the existing state of the art in
    Section~\ref{sec:test:state_of_art}. We then describe our contribution in Section~\ref{sec:test:contribution}. We
    start by detailing the statistics on which rely our tests. Then, we describe how we implemented these tests, from
    the experiment execution to the final test result. We show and explain the graphical representation of our test
    results, and we describe the various events we were able to detect on the Grid'5000 platform. Finally, in
    ...
    \label{sec:test:state_of_art}
        ...
        \begin{itemize}
            ...
            \item The result of a benchmark is a raw performance value (\eg a duration, an amount of memory or an amount
                ...
                failed. It is difficult to define properly such a \emph{ground truth}. One could define an interval for
                the expected value, but this will most of the time be an arbitrary choice. A complementary solution, to
        ...
        \label{sub:lack_of_tests}
            ...
            implemented\footnote{\url{https://github.com/xianyi/OpenBLAS/tree/f917c26e/benchmark}}, but they rely on human
            intervention to run the tests and interpret the performance results. OpenMPI has a software, named
            MTT\footnote{\url{https://open-mpi.github.io/mtt/}}, to automatically deploy a middleware on an infrastructure and
        ...
        \label{sub:existing_work}
            ...
            \subsubsection{Benchmarking software: testing code snippets}%
                ...
                well-established with several dozens of contributors and about \Num{5000} stars on GitHub. It greatly
                ...
                script\footnote{\url{https://github.com/google/benchmark/blob/8f5e6ae0/tools/compare.py}} to compare the
                ...
                \begin{itemize}
                    ...
                    \item We do not have any information on the distribution of these durations, only the average is
                        ...
                        but this is more problematic in our use case where we measure longer function calls that take at
                        least several milliseconds.
                    \item There is no support for randomization as the code snippets are executed in a deterministic
            ...
            \subsubsection{Benchmarking software: long-term performance evolution of an application}%
                ...
                applications and measures their performance, both in reality and in simulation. The goal is to detect any
                performance regression possibly introduced by a change in the software.
                ...
                large jump on 15/12/2017 is due to a hardware upgrade. The real runs exhibit a large
                ...
                platform, these simulations are extremely valuable to them, allowing them to rule out the false conclusions
                ...
                satisfying to some extent, it raises two concerns:
                \begin{itemize}
                    ...
                    \item This does not scale well, adding more applications or running the test on more machines would
                        make this visual inspection very tedious.
                \end{itemize}
            ...
            \subsubsection{Benchmarking systems}%
                ...
                nodes~\cite{nussbaum:hal-01538682}. These are mostly \emph{correctness} tests. They perform several
    ...
    \section{Performance non-regression statistical testing}%
    \label{sec:test:contribution}

        \subsection{Statistical test basis}%
        \label{sub:statistical_test}
            ...
            then be used to implement a statistical test.  This section is mostly based on the work by
            ...
            independent, they have a correlation coefficient of -0.7. The \NSI{99.5}{\percent} prediction regions are
            ...
            one-dimension density plots and they both fall within the \NSI{99.5}{\percent} interval. Now, when we look
            at both dimensions simultaneously, the green point becomes much more likely to be observed as it is within the
        ...
        \label{sub:workflow}
            ...
            This section describes the different steps and tools involved to perform the performance test described in
            Section~\ref{sub:statistical_test}, from the
            ...
            The workflow, illustrated in Figure~\ref{fig:experiment:g5k_test:workflow}, consists of three main steps.
            The first one is implemented with \peanut while the two others are implemented with \cashew~\cite{cashew}, a
            Python library implemented specifically for this task.
            \begin{description}
                ...
                \item[Archive extraction and aggregation] The data is extracted from the archive and appended to two
                    HDF5 files (one for the \dgemm duration, one for the monitoring data). Then, some
                    statistics are computed with this new data on a per-CPU and per-experiment basis (\eg average \dgemm
                    performance and average CPU temperature), these aggregated values are added to two CSV
                    files.
                \item[Statistical test computation] Several statistical tests are done on the sequence of aggregated
                    data. The result is presented as several Jupyter notebooks containing plots.
            \end{description}


            \subsubsection{Experiment execution}%

                The experiment script is written in Python and uses the \peanut experiment engine~\cite{peanut} (see
                ...
                we should postpone our experiment to avoid disturbing too much the other users. A particularly good
            ...
            \subsubsection{Statistical test computation}%
                ...
                test does not need the full repository, instead it automatically downloads the required files.
                ...
                As for the previous step, the test is implemented with \cashew and uses a Jupyter notebook. This notebook
        ...
        \label{sub:presentation_of_the_test_results}
            ...
            \subsubsection{Evolution plot}%
                ...
                \begin{figure}[htpb]
                    \centering
                    \subfigure[Performance evolution of node \dada{14}.\label{fig:experiment:non_regression:evolution_dahu14}]{
                        \includegraphics[width=\linewidth]{img/experiment/non_regression/implementation/evolution_dahu-14.pdf}
                    }
                    \subfigure[Performance evolution of node \dada{26}.\label{fig:experiment:non_regression:evolution_dahu26}]{
                        \includegraphics[width=\linewidth]{img/experiment/non_regression/implementation/evolution_dahu-26.pdf}
                    }
                    \caption{
                        Evolution of the mean performance of the two processors of two nodes from cluster \dahu
            ...
            \subsubsection{Overview plot}%
                ...
                We are running regular tests for several factors on hundreds of nodes, therefore it would be very long and tedious
            ...
            \subsubsection{Windowed test}%
                ...
                permitting to detect much more subtle changes. The downside is that it introduces a lag, meaning that
                the change might not be detected immediately.

                \begin{figure}[htpb]
                    \centering
                    \subfigure[Performance evolution of node
                        \dada{14}.\label{fig:experiment:non_regression:evolution_dahu14_windowed}]{
                        \includegraphics[width=\linewidth]{img/experiment/non_regression/implementation/evolution_dahu-14_windowed.pdf}
                    }
                    \subfigure[Performance evolution of node
                        \dada{26}.\label{fig:experiment:non_regression:evolution_dahu26_windowed}]{
            ...
            \subsubsection{Multi-factor test}%
                ...
                \begin{itemize}
                    ...
                    \item Two events have gone completely unnoticed by the multi-factor test whereas they were reported
                        by the single-factor test: the positive anomaly on \dahu[26] from 2020/05/18, and the series of
                        negative anomalies (that we later decided to be outliers) between 2020/11 and 2020/12 on nodes
                        \dahu[29], \dahu[30], \dahu[31] and \dahu[32]. The reason they went
                        ...
                        so, the multi-factor test is able to detect the anomaly from 2020/05/18 on \dahu[26], but
            ...
            \subsubsection{Website presentation}%
                ...
                \begin{figure}[htb]
                    \centering
                    \fbox{\includegraphics[width=0.8\linewidth]{img/experiment/non_regression/implementation/g5k_test_home.png}}
                ...
                \begin{figure}[htb]
                    ...
                    \end{tikzpicture}}
                    \caption{Overview of a notebook (cluster \gros, average performance).}%
                ...
                \begin{enumerate}[label=\alph*]
                    ...
                    \item This is an overview plot of the cluster, similar to the overview plots previously presented.
                        ...
                        again the utility of statistical tests: only the most abrupt changes are visible in this plot,
        ...
        \label{sub:detected_events}
            ...
            Our non-regression tests have been used on a regular basis on twelve Grid'5000 clusters during a period of
            several months (from July 2019 to March 2021 for \dahu, the first cluster we started to monitor).
            In total, we have tested 454 nodes (792 processors).
            ...
            \subsubsection{BIOS upgrade}%
                ...
                \begin{itemize}
                    ...
                    \item On 2020/04/01, cluster \dahu. The upgrade caused a performance drop of \NSI{5}{\percent}
                        on node \dahu[26], as well as a decrease of its frequency and temperature. There was also a
                        statistically significant (albeit very small) change on the other nodes of the cluster (T-test
                        with a \NSI{95}{\percent} confidence). This issue on \dahu[26] has later been resolved by
                    ...
                    \item On 2020/09/29, cluster \troll. A very large temperature drop followed the
                        upgrade. The largest effect was on CPU n°1 of node \troll[2], where the temperature
            ...
            \subsubsection{Cooling issue}%
                ...
                Four nodes from cluster \dahu encountered some issues in two occasions, namely \dahu[13],
                \dahu[14], \dahu[15] and \dahu[16]. Their performance and frequency was lower by
                \NSI{10}{\percent} whereas their temperature was much higher, especially on their CPU n°0 where it went
                above \NSI{90}{\celsius} instead of the usual \NSI{70}{\celsius}. Our hypothesis is that for some
            ...
            \subsubsection{Faulty memory}%
                ...
                \begin{itemize}
                    \item Node \yeti[3] on 2019/07/05.
                    \item Nodes \dahu[20] and \dahu[24] on 2019/08/14.
                    \item Nodes \dahu[20] and \dahu[22] on 2019/11/03.
                    \item Node \dahu[7] on 2020/09/17 and on 2020/09/28.
                    \item Node \dahu[14] on 2020/09/28.
                \end{itemize}

                The Grid'5000 team was able to solve this problem, sometimes by swapping two memory sticks, sometimes
                by replacing one memory stick, sometimes by simply performing a power drain of the nodes.
            ...
            \subsubsection{Power instability}%
                ...
                \NSI{119.4}{\watt}. The largest power drop happened on node \dahu[26] on 2020/12/04, where its CPU
                n°0 consumed only \NSI{97.5}{\watt}. For reasons we ignore, such power drops happened particularly
                frequently on the four nodes \dahu[29], \dahu[30], \dahu[31] and \dahu[32] between
                ...
                When booted but idle, a single node has a power consumption between \NSI{600}{\watt} and
                \NSI{800}{\watt}, this consumption can rise to more than \NSI{2000}{\watt} when the nodes are under
                heavy load. This is a large consumption compared to other Grid'5000 nodes (the \dahu nodes consume about
                \NSI{350}{\watt} at most).
            ...
            \subsubsection{System upgrade}%
                ...
                \begin{itemize}
                    \item The average performance increased from \NSI{27}{\giga\flop/\second} per core to
                        \NSI{45}{\giga\flop/\second}.
                    \item The average frequency decreased from \NSI{2.2}{\giga\hertz} to \NSI{1.8}{\giga\hertz}.
                    \item The mean power consumption of the DRAM increased from \NSI{12}{\watt} to \NSI{15}{\watt}.
                ...
                6130 CPU (the processor of the \dahu nodes) has a maximal frequency of \NSI{2.8}{\giga\hertz} when all
                cores are active with normal instructions, \NSI{2.4}{\giga\hertz} when all cores are active with AVX2
                instructions and \NSI{1.9}{\giga\hertz} when all cores are active with AVX512
            ...
            \subsubsection{Other issues}%
                ...
                The two nodes \parasilo[1] and \parasilo[11] also had a temperature increase of
                \NSI{5}{\celsius} a few weeks later, on 2020/10/06. This new change was not enough to revert the effect
                of the first change.

                The two nodes \dahu[29] and \dahu[30] had a temperature increase a few weeks later and went
    ...
    \label{sec:test:conclusion}
        ...
        best of our knowledge, we are the first to apply these statistics for testing computer performance.
        ...
        There remains some engineering work before targeting a broader adoption. Some parts of our workflow should be
