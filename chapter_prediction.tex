...
\label{chapter:prediction:related_work}
    ...
    In this chapter, we discuss the different techniques and tools available for predicting the performance of an
    application. Section~\ref{sec:performance_prediction} explains why we need simulation to obtain both efficient and
    faithful predictions while Section~\ref{sec:simgrid_smpi} gives an overview of the simulator we used for this work.
    ...
    \label{sec:performance_prediction}
        ...
        on a given platform. The objectives are diverse: to compare several algorithms, to verify if the
        ...
        \begin{description}
            \item[Extrapolation on the problem size] Most of the applications can take as input problems of various
                ...
                whether the considered approach can predict the performance of the application for an arbitrary problem
                size.
            \item[Extrapolation on the configuration] It is often possible to tune the behavior of the application with
                ...
                size. This criterion designates if the approach can make predictions for an arbitrary parameter
                combination.
            \item[No full-scale real run] Some prediction methods require at least one run of the application at the
                desired scale in terms of number of nodes. This criterion denotes whether the approach can avoid making
                such expensive runs (note that the full-scale runs do not necessarily need to be done on the target
                platform).
        ...
        accurate estimates. For instance, complex phenomena like network contention cannot be captured faithfully by
        these models.
        ...
        Another approach for estimating the performance of parallel applications implemented using the Message Passing
        Interface (MPI) standard is statistical modeling of the application
        as a whole~\cite{hpl_prediction}.  By running the application several times for small- and medium-sized problem (or a
        few iterations of large problem sizes) and using simple linear regressions, it is possible to predict its
        execution time for larger sizes with an error of only a few percents and a relatively low cost. Unfortunately, the
        ...
        accurate predictions at a low cost, but it was not possible to predict the execution time with an arbitrary problem
        size. Furthermore, such approaches cannot be used to study what-if scenarios (\eg to evaluate what
        ...
        Simulation provides the level of details and flexibility that is lacking in such black-box modeling approaches.
        Performance prediction of MPI applications through simulation has been widely studied over the last decades but
        two approaches can be distinguished in the literature: offline and online simulation.
        ...
        The second approach discussed in the literature is \emph{online simulation}.  Here, the application execution is
        emulated on top of a simulator that is responsible for simulating the execution of each application process
        including inter-process communication. This approach allows
        ...
        established, but the proposed developments would also be possible with SST.  Note that the emulation
    ...
    \label{sec:simgrid_smpi}
        ...
        models particularly for scenarios making heavy use of the network. Model validation usually consists of comparing
        ...
        The accuracy of simulations conducted with SMPI has been reported in the literature for several applications.
...
\label{chapter:prediction:hpl}
    ...
    This chapter gives a thorough overview of the application we used as a case study for this work, namely High
    Performance Linpack (HPL). It presents the different state-of-the-art techniques employed in HPL to reach the
    highest possible performance, making it more challenging to simulate accurately.
    ...
    \label{sec:hpl:benchmark}
        ...
        \end{figure}
        In this work, we use the freely-available reference-implementation of HPL~\cite{hpl} as our main target
        application. HPL is a parallel application implemented with MPI (Message Passing Interface), an established
        standard in the industry~\cite{mpi_standard}. HPL implements a matrix factorization based on a right-looking
        variant of the LU factorization with row partial pivoting and allows multiple look-ahead depths. The principle
        of the factorization is depicted in Figure~\ref{fig:hpl_overview}. It consists of a series of panel
        factorizations followed by an update of the trailing sub-matrix.  HPL uses a two-dimensional block-cyclic data
        distribution of $A$ and implements several custom MPI collective communication algorithms to efficiently overlap
        ...
        \begin{itemize}
            ...
            \item \texttt{DEPTH} controls how many iterations of the outer loop overlap with each other.
            \item \texttt{BCAST} sets the algorithm used to broadcast a panel of columns over the process columns. Legacy
                ...
                \newcommand\scalingbcast{0.9}
                \begin{figure}[htpb]
                    \centering

                    % 1ring
                    \scalebox{\scalingbcast}{\subfigure[Broadcast \texttt{ring}.]{\begin{tikzpicture}[>=latex',line join=bevel,]
                        \node (a) [mpi_rank] {a};
                    ...
                    % 1rinM
                    \scalebox{\scalingbcast}{\subfigure[Broadcast \texttt{ring-modified}.]{\begin{tikzpicture}[>=latex',line join=bevel,]
                        \node (a) [mpi_rank] {a};
                        \node[right of=a] (b) [mpi_rank] {b};
                        \node[right of=b] (c) [mpi_rank] {c};
                        \node[right of=c] (d) [mpi_rank] {d};
                    ...
                    % 2ring
                    \scalebox{\scalingbcast}{\subfigure[Broadcast \texttt{2-ring}.]{\begin{tikzpicture}[>=latex',line join=bevel,]
                        \node (a) [mpi_rank] {a};
                        \node[right of=a] (b) [mpi_rank] {b};
                        \node[right of=b] (c) [mpi_rank] {c};
                        \node[right of=c] (d) [mpi_rank] {d};
                    ...
                    % 2rinM
                    \scalebox{\scalingbcast}{\subfigure[Broadcast \texttt{2-ring-modified.}]{\begin{tikzpicture}[>=latex',line join=bevel,]
                        \node (a) [mpi_rank] {a};
                        \node[right of=a] (b) [mpi_rank] {b};
                        \node[right of=b] (c) [mpi_rank] {c};
                        \node[right of=c] (d) [mpi_rank] {d};
                ...
                \begin{figure}[htpb]
                    \centering
                    % long (scatter phase)
                    \scalebox{\scalingbcast}{\subfigure[Broadcast \texttt{long}, scatter phase.\label{fig:hpl_broadcasts:long:scatter}]{
                    ...
                    \scalebox{\scalingbcast}{\subfigure[Broadcast \texttt{long}, roll phase.\label{fig:hpl_broadcasts:long:roll}]{
                        \newcommand{\drawif}[2]{
                            \unless\ifnum #1<#2
                            1
                            \else
        ...
        The \emph{panel} mentionned in Figure~\ref{fig:hpl_overview} is an intricate data structure used by
        ...
        estimation of HPL's execution time with an analytical model, but more accurate predictions require the simulation of the
        application. The sequential time complexity of HPL's factorization is \[\mathrm{flop}(N) = \frac{2}{3}N^3 + 2N^2 +
    ...
    \label{sec:hpl:typical_runs}
        ...
        One goal of this thesis is to simulate systems at the scale of Stampede. Given the scale of this scenario
        (\Num{3785}~steps on \Num{6006}~nodes in two hours), detailed simulations quickly become intractable without
        significant effort. The simulation of HPL therefore comes with at least two challenges:
        \begin{itemize}
            ...
            \item The execution of HPL at scale leads to enormous memory consumption. It is therefore not possible to
                conduct executions at scale on a single or even a few compute nodes.
        \end{itemize}

\chapter{Simulating HPL at large scale}%
\label{chapter:prediction:emulation}
    ...
    \label{sec:speeding_emulation}
        ...
        \subsection{Compute Kernel Modeling}
            ...
            solving an  \(A\cdot x=b\) equation). The analysis of an HPL execution with \(64\) processes and a very
            ...
            Skipping kernels renders the content of some variables invalid but in simulation, only the performance behavior of the
            ...
            through benchmarking. The estimated time of the kernel is calculated based on the given parameters and passed on
            to \texttt{smpi\_execute\_benched} which advances the clock of the executing process by this estimate.  The effect
            on the simulation time for a small scenario is depicted in Figure~\ref{fig:kernel_sampling}. This modification
            speeds up the simulation by orders of magnitude. The impact on the accuracy of the simulation will be investigated in more
            details in the next chapter, but it can already be observed that this simple kernel model leads to a sound,
            albeit slightly more optimistic, estimation of the execution time.

            In addition to the main compute kernels, a profiling of the code made it possible to identify seven other BLAS functions
            ...
            the LU factorization and hence impact the performance of the HPL execution; however, because of the removal of the
            \dgemm and \texttt{dtrsm} computations, they all operate on invalid data and hence also produce invalid
            ...
            (\(N=\Num{30000}\)) and even more on larger scenarios.

        \subsection{Specific Adjustments}
            HPL uses pseudo-randomly generated matrices that are set up for every execution. This initialization, just
    ...
    \section{Scaling Down Memory Consumption}
        ...
        Unfortunately, when simulating an application with SMPI, all MPI processes are run within the same simulation
    ...
    \section{Scalability Evaluation}
        ...
        should be decreased to something independent of \(N\). SimGrid's fluid network models, used to simulate communications,
        ...
        To evaluate the efficiency of our proposal, we conduct a first evaluation on a non-existing but Stampede-like
        platform comprising 4,096 nodes interconnected through a fat-tree topology.
        ...
        Stampede scenario.  The impact of the matrix order on total execution time and memory is illustrated in
...
\label{chapter:prediction:modeling}
    ...
    the actual performance can vary wildly depending on the range of the message size as MPI switches from one
    protocol to another based on message size thresholds. In this chapter we first introduce some notations to describe the complexity of
    ...
    \section{Modeling notations}%
        ...
        \begin{itemize}
            \item Modeling option \model{0}: For simple compute kernels, the duration can be modeled as a
        ...
        observed on real platforms. It may be common to have a few operations being one order of magnitude slower than
        all the other ones even though they had the same parameter set. In this case, a reasonable option consists in modeling noise
    ...
    \label{sec:dgemm_model}
        ...
        \label{sub:dgemm_model:different_models}
            ...
            \dgemm with randomized input sizes over all the cores of each node (to reproduce experimental conditions
            ...
            likely to be rather important for simulating the execution of HPL.
            Second, we took care of covering a wide variety of combinations for
            \(M\), \(N\), and \(K\). It can be observed that \(M.N.K\) is not sufficient to describe correctly the
            performance of \dgemm, even though the number of operations performed by this function is expected to be
            proportional to \(M.N.K\). Indeed, for \(M.N.K \approx \num{4.5E9}\) some duration are systematically
            ...
            total duration of these kernels is minimal compared to the total execution time, they may perturb
        ...
        \label{sub:dgemm_model:generative_models}
            ...
            \begin{equation}
                ...
                \end{split}
            \end{equation}
            where \(\mathcal{H}(\mu, \sigma)\) denotes a random variable that follows a half-normal distribution with
            ...
            Finally, we initially decided to use a half-normal distribution instead of a normal distribution to account
            for the asymmetry of the observed durations (\eg there are no negative durations), but with hindsight we do
            not believe this choice to be very important.
            ...
            iii) a ``long term'' temporal variability (from one day to another). As illustrated in
            Chapter~\ref{chapter:prediction:validation}, accounting for the first two kinds of variability is essential
            but during our investigation of the simulation validity, which spanned several months, we also had to
            deal with the fact that the node performance from one day to another could significantly vary, thereby making
            our comparisons between a real experiment and the simulation driven by models derived from past measurements
            ...
            this variability was different from one machine to another, hence, \(\Sigma_T\) is not indexed by \(p\) but
            global to all machines. However, the parameters \(\alpha_{p,d},\beta_{p,d},\gamma_{p,d}\) are generally
            correlated to each others, hence \(\Sigma_T\) is the full covariance matrix to account for interactions.  The
            ...
            statistically significant (most ellipses are slightly tilted toward the North-East), hence a full variance
            ...
            Given the above, it is easy to estimate \(\mu_{p}\) and \(\Sigma_T\) by averaging over the \(\mu_{p,d}\) of each node,
            ...
            orders of magnitude of all parameters and the ellipses are excellent, a few differences are visible.
            ...
            very similar to the original measurements.
    ...
    \label{sec:network_model}
        ...
        \subsection{Modelization in Simgrid}%
            ...
            variability could explain some (overall poor) performance since they generally get amplified by broadcast and
            pipelined communication patterns.

            Automatically computing the best fit for a complex model like the aforementioned one is much more difficult
            than for the \dgemm model proposed in Section~\ref{sec:network_model}. It should be possible to compute it
            in one go with a Bayesian framework like STAN, but it would likely be computationally prohibitive. A more
            ...
            \begin{enumerate}
                ...
                \item We tested an existing R package named \texttt{flexmix} for computing a linear regression with a
                    Gaussian mixture noise~\cite{flexmix}. It proved to be fragile, the result being
                    non-deterministic: the number of modes found by the algorithm was not constant. We ended up
                    clustering the data manually via visual inspection and then computing a simple linear regression
                    for each mode.
            \end{enumerate}
        ...
        \label{sub:finding_semantic_breakpoints}
            ...
            In network calibration experiments, \ie the experiments carried to measure the network performance and
            instantiate a model, we can observe sudden performance changes, \aka breakpoints (see
            Figure~\ref{fig:nw_var}).  As discussed previously, these breakpoints happen because of a protocol change in
            MPI implementations. Some of these changes also affect the semantic of the \send operation: we described in
            ...
            section as well as Section~\ref{sub:pycewise}, MPI is seen as a black box and we use reverse-engineering
            ...
            We define two MPI codes, Algorithm~\ref{algo:semantic:deadlock} and Algorithm~\ref{algo:semantic:corruption}.
            They both take in input a message size \texttt{N}. We expect Algorithm~\ref{algo:semantic:deadlock} to produce
            ...
            expect Algorithm~\ref{algo:semantic:corruption} to have corrupted data and print the message ``CORRUPTED''
            ...
            We repeatedly call the Algorithms~\ref{algo:semantic:deadlock} and~\ref{algo:semantic:corruption},
            performing a binary search on the argument \texttt{N} (we set a timeout of \NSI{3}{\second} to kill
            Algorithms~\ref{algo:semantic:deadlock} in case of deadlock). This allows to find the two semantic breakpoints
            easily:
            \begin{itemize}
            ...
            Note that in Algorithms~\ref{algo:semantic:corruption}, the function \isend is used instead of \send. Indeed,
            ...
            In our tests, we found that the three protocols did not always exist, and that the threshold will depend on the
            hardware. For instance, on the \dahu cluster, the semantic breakpoints found by the two binary searches are equal.
            This means that the \emph{detached} mode does not exist in the MPI implementation installed on this cluster
            (for a given size \texttt{N}, either we have both a deadlock with Algorithms~\ref{algo:semantic:deadlock}
            and a corruption with
            Algorithms~\ref{algo:semantic:corruption}, or we have none, there is no alternative combination).
            On clusters \paravance and \grisou, the search with Algorithm~\ref{algo:semantic:corruption} does not find any
            breakpoint, the message is always corrupted. This means that the \emph{asynchronous} mode does not exist.

            These kinds of inconsistencies show the fragility of the semantic approach for detecting breakpoints. The
            three behaviors we described, namely \emph{asynchronous}, \emph{detached} and \emph{synchronous}, are not
            part of the MPI standard. They highly depend on both the MPI implementation and the hardware.
            For instance, Algorithm~\ref{algo:semantic:corruption} depends on the semantic of a concurrent memory read
            and write, which can produce surprising results on less common architectures. An alternative would be to
            instrument the memory of the program to have a precise timing of when the memory reads and writes occur.
        ...
        \label{sub:pycewise}
            ...
            \subsubsection{Model and notations}%
                ...
                (\eg the message size) will be denoted \(X\), the response variables (\eg the communication duration) will
                be denoted \(Y\). We will denote \(\ell\) the list of tuples \((X,Y)\). We assume that there are \(N\) different
                unknown breakpoints \(B_1 < \dots < B_N\), where \(N\) itself is also unknown. For convenience, we will
                denote \(B_0=-\infty\) and \(B_{N+1} = +\infty\).  Then, there are \(N+1\) tuples \((\alpha_i, \beta_i,
                ...
                and tuples \((\alpha_i,\beta_i)\). The values \((\gamma_i, \delta_i)\) are not computed here, but they could
                ...
                The concatenation of lists will be denoted \(\ell_1 \bullet \ell_2\), hence
                \(\text{BIC}(\ell_1\bullet\ell_2)\) will denote the BIC of a linear regression without breakpoint
                (\(N=0\)).

            \subsubsection{Main algorithm}%

                The algorithm proceeds in two steps: it starts by a top-down step where new breakpoints are
                ...
                given call, we iterate on the list to test all the \(X\) values as a possible breakpoint: we search for the
                ...
                In the next sections, we change the objective function and have to drop this ability of doing an
                ...
                left. In the end, we keep the iteration that minimizes the BIC.
                \begin{algorithm}
                    \DontPrintSemicolon
            ...
            \subsubsection{Comparison of the solutions}%
                ...
                for our context that has characteristics similar to that of the network calibration data (see
                ...
                Figure~\ref{fig:prediction:pycewise_demo}. We do not show results for the alternative regression methods here
                because, like for the memcpy experiment, they do not work properly.
                We executed our MPI calibration program on four clusters of
                ...
                evaluating the quality of the fit. Computing the optimal solution that minimizes the objective function
    ...
    \section{Conclusion}%
        ...
        uncertainty of the resulting parameters. However, the benefit of these tools is not clear for our context as
        their computation time is particularly long. Furthermore, we had great difficulties in writing down our models
        in their formalism. In the end, a simpler approach like the method of moments revealed to be enough for our
        needs.
...
\label{chapter:prediction:validation}
    It is not possible to formally prove the ``correctness'' of a model like we would do for a theorem. The only
    reasonable method to evaluate the soundness of our approach is to compare simulation to reality. The goal here is to
    meticulously try many relevant configurations of the application to possibly unveil any flaw of the models discussed
    previously.

    \section{Experimental setup}%
    \label{sec:experimental_setup}
        We used the \dahu cluster from the Grid'5000 testbed. It has 32 nodes connected through a
        ...
        also used the libraries OpenMPI version 2.0.2 and OpenBLAS version 0.3.1. Last, we used one single-threaded
        MPI rank per core.
    ...
    \label{sec:internal_behavior}
        ...
        that harm the efficiency of collective operations.

        This demonstrates one of the advantages of simulation over other prediction methods. By generating a trace, we
        can have a very thorough visual inspection of the simulated execution, which makes it possible to identify the
        culprits of accuracy losses.
        ...
        \begin{figure}[htpb]
            ...
            \ganttcaption{Simple kernel (\model{1}\noise{0})}{Simple network (\modelp{1}\noise{0})}    &
            ...
            \ganttcaption{Complex kernel (\model[H]{2}\noise[H]{2})}{Simple network (\modelp{1}\noise{0})}   &
            ...
            \ganttcaption{Complex kernel (\model[H]{2}\noise[H]{2})}{Complex network (\modelp{1}\noisep{1})}  &
    ...
    \label{sec:different_problem_sizes}
        ...
        Figure \ref{fig:blas_var} shows that there is a significant heterogeneity in the cluster. For this reason, we
        ...
        variability is however a very important aspect to model applications that are more \emph{latency-bound}.
    ...
    \label{sec:platform_change}
        ...
        upgraded. During this time frame, the cluster has also suffered from hardware issues, such as a cooling malfunction
        ...
        \begin{figure}[htpb]
            ...
            \begin{minipage}{0.2\linewidth}
                ...
                \end{tabular}}
            \end{minipage}
            \caption{HPL performance: predictions vs. reality (effect of the cooling issue on the nodes
            \dahu[\{13,14,15,16\}]).}
        ...
        conducting a new calibration whenever a significant change is detected.  This ability to reflect in simulation a
    ...
    \label{sec:different_geometries}
        ...
        \begin{figure}[htb]
            ...
            \caption{The first (optimistic) network calibration gives poor
        ...
        Equation~\eqref{eq:dgemm.complex} (stochastic, heterogeneous, and polynomial) and the simplest linear models for the
        ...
        Figure~\ref{fig:mpi_calibration}, we obtained the unsatisfactory orange line on top of
        ...
        We address the above problem by improving our network calibration procedure: (1) we use a distinct model for local
        ...
        structure of the collective operations but requires a close inspection of the code.
    ...
    \label{sec:factorial_experiment}
        ...
        Although geometry is among the most important parameters to tune, six other parameters control the behavior of
        ...
        \(\text{P}\times\text{Q}=32\times32=1024\) as it is best experimentally (the simpler calibration procedure and the network
...
\label{chapter:prediction:sensibility}
    ...
    \label{sec:influence_of_temporal_variability}
        In Section~\ref{sec:different_problem_sizes}, we were able to highlight the importance of accounting for temporal
        ...
        fully quantified it since designing and performing real experiments to evaluate it would be quite difficult.
        Although increasing this variability would be feasible, reducing it would be particularly complicated. This
        can however easily be done through simulation using the hierarchical model of
        Section~\ref{sub:dgemm_model:generative_models}. In our
    ...
    \label{sec:influence_of_spatial_variability}
        ...
        about \NSI{9}{\percent} of performance degradation, spatial variability was even more important as it was responsible for
        ...
        nodes cannot help improve performance. Note that the overhead for \(5\times\texttt{Q}\) configurations with a
...
\label{chapter:prediction:conclusion}
    ...
    platform is useful for application developers and users. It will become invaluable in the future as it can, for
    ...
    modeled accurately, which allows us to systematically predict the overall performance of HPL within a few percent. Our
    ...
    of mitigation strategies (e.g., the eviction of the slower nodes). Likewise, the simulation makes it possible to
    assess precisely the influence of the physical network on the overall performance. Most of these \emph{what-if}
    studies would be particularly difficult to conduct through real experiments because of the difficulty to finely
    control the platform.  This is to the best of our knowledge one of the first sensitivity analyzes of a real HPC code
    accounting for platform uncertainty.
