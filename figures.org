...
draw_plot <- function(center, shape, radius, new_points, nb_points=1000, seed=42) {
    set.seed(seed)
    ...
    scatter_plot = base_plot +
        ...
        geom_path(data=calculate_ellipse(center, shape, radius, 100), colour=base_col) +
    ...
    plot_top = base_plot +
        ...
        stat_function(fun = dnorm, n = 100, args = list(mean=center[1], sd=std[1]))
    ...
    plot_right = base_plot +
        ...
        stat_function(fun = dnorm, n = 100, args = list(mean=center[2], sd=std[2])) +
        coord_flip()
...
tmp = mvrnorm(n=nb_points, mu=mu, Sigma=sigma)
new_good = data.frame(x=tmp[,1], y=tmp[,2], type="good")
tmp = mvrnorm(n=nb_points, mu=mu*1.1, Sigma=sigma)
new_bad = data.frame(x=tmp[,1], y=tmp[,2], type="bad")
new_points = rbind(new_good, new_bad)

plot = draw_plot(mu, sigma, radius_multiple, new_points)
#ggsave(filename='img/experiment/non_regression/statistics/single_point.pdf', plot=plot, width=7, height=5)
plot
#+end_src

#+RESULTS:
[[file:/tmp/babel-LlZHec/figurec3Fp8E.png]]
