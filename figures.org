...
plot = plot + geom_rect(data=base_df, xmin=240000, xmax=260000, ymin=19700, ymax=22000, linetype="dashed", alpha=0.0, fill="black", color="black")
plot
tikzTest("A1het")
dev.off()
print(plot)
...
Running command: '/usr/bin/pdflatex' -interaction=batchmode -halt-on-error -output-directory '/tmp/RtmpW8AQle/tikzDevice4e8211a3bd7' '/tmp/RtmpW8AQle/tikzDevice4e8211a3bd7/tikzStringWidthCalc.tex'
...
#+RESULTS:
[[file:/tmp/babel-sQb4pJ/figureukCpIh.png]]
**** Multiple points
#+begin_src R :results output graphics :file (org-babel-temp-file "figure" ".png") :exports both :width 600 :height 400 :session *R*
set.seed(41)
...
plot = draw_plot(mu, sigma, radius, new_obs, new_point_size=3)
ggsave(filename='img/slides/several_points.pdf', plot=plot, width=7, height=5)
...
#+RESULTS:
[[file:/tmp/babel-sQb4pJ/figureF3Hr9H.png]]
...
plot = ggplot() +
    ...
    annotate("label", x=center, y=0.45, label="bar(x)", size=12, label.padding=unit(0.8, "lines"), parse=T) +
    annotate("label", x=center-2*std, y=0.45, label="bar(x)-2*s", size=8, color=base_col, label.padding=unit(0.8, "lines"), parse=T) +
    annotate("label", x=center+2*std, y=0.45, label="bar(x)+2*s", size=8, color=base_col, label.padding=unit(0.8, "lines"), parse=T) +
...
#+RESULTS:
[[file:/tmp/babel-Y1PGmR/figureuKRYEI.png]]
* Conclusion
** Validation on Gros cluster
The CSV is stored locally. The experiments were done during the weekend of
[2021-05-01 Sat], the data has been dumped into this CSV file on
[2021-06-17 Thu]. The notebooks and archives are located in the repository of my
journal, see the corresponding entries in the journal for more information.
*** Drawing the plot
#+begin_src R :results output :session *R* :exports both
library(ggplot2)
library(dplyr)
options(warn=-1)
df = read.csv('data/conclusion/hpl_matrix_size.csv') %>%
    mutate(mode = as.character(mode)) %>%
    mutate(mode = ifelse(mode=="Simulation (simple)", "Simulation (simple dgemm model)",
                  ifelse(mode=="Simulation (complex)", "Simulation (complex dgemm model)", mode)))
head(df)
#+end_src

#+RESULTS:
: 
:   matrix_size gflops    time                            mode       date   jobid
: 1       50000   4660   17.88 Simulation (simple dgemm model) 2021-05-02 2989432
: 2      350000  25730 1110.89 Simulation (simple dgemm model) 2021-05-02 2989438
: 3      200000  20420  261.23 Simulation (simple dgemm model) 2021-05-02 2989435
: 4      150000  17220  130.65 Simulation (simple dgemm model) 2021-05-02 2989434
: 5      450000  27630 2198.58 Simulation (simple dgemm model) 2021-05-02 2989440
: 6      400000  26580 1605.43 Simulation (simple dgemm model) 2021-05-02 2989439

Then, we can generate the plot.

#+begin_src R :results output :session *R* :exports both
lab = df %>% filter(
    (mode == 'Reality'                          & matrix_size == 300000) |
    (mode == 'Simulation (simple dgemm model)'  & matrix_size == 200000) |
    (mode == 'Simulation (complex dgemm model)' & matrix_size == 150000)
) %>%
    select(gflops, matrix_size, mode) %>%
    group_by(matrix_size, mode) %>%
    summarise(gflops=mean(gflops)) %>%
    as.data.frame()
...
delta_x = max(df$matrix_size) / 12
delta_y = max(df$gflops) / 12
max_x = max(df$matrix_size)
max_y = max(df$gflops)
lab = lab %>% mutate(xl = case_when(mode == 'Reality'     ~ 0.7*max_x,
                                    mode == 'Simulation (simple dgemm model)' ~ 0.25*max_x,
                                    mode == 'Simulation (complex dgemm model)' ~ 0.4*max_x,
                                       T             ~ xl))
lab = lab %>% mutate(yl = case_when(mode == 'Reality'     ~ 0.5*max_y,
                                    mode == 'Simulation (simple dgemm model)' ~ 0.9*max_y,
                                    mode == 'Simulation (complex dgemm model)' ~ 0.4*max_y,
                                       T             ~ yl))
lab
#+end_src
...
`summarise()` regrouping output by 'matrix_size' (override with `.groups` argument)

  matrix_size                             mode  gflops      x       y     xl
1      150000 Simulation (complex dgemm model) 16120.0 150000 16120.0 200000
2      200000  Simulation (simple dgemm model) 20420.0 200000 20420.0 125000
3      300000                          Reality 23257.5 300000 23257.5 350000
     yl
1 11256
2 25326
3 14070
...
aes(x=matrix_size, y=gflops, color=mode, fill=mode, shape=mode) +
        ...
        expand_limits(y=0) +
        scale_shape_manual(values=c(23, 21, 21)) +
        geom_segment(data=lab, aes(x=xl, xend=x, y=yl, yend=y, color=mode), linetype="solid") +
        geom_label(data=lab, aes(x=xl, y=yl, label=mode, group=mode), size=text_size, fill='white') +
        geom_label(data=lab, aes(x=xl, y=yl, label=mode, group=mode, fill=mode), alpha=0.3, size=text_size, color="black") +
        ylim(0, 30000) +
        ylab('Gflop/s') +
        xlab('Matrix rank') +
        labs(color="Mode", fill="Mode", shape="Mode") +
...
ggsave(filename='img/conclusion/validation_gros.pdf', plot=plot)
plot
#+end_src

#+RESULTS:
[[file:/tmp/babel-rThAwc/figurex95lAi.png]]
...
** CO2 emissions repartition (slides)
#+NAME: co2_thesis
| source       | idx |  co2 | comment              |
|--------------+-----+------+----------------------|
| Flights      |   1 |  7.6 | 2 trips to the USA   |
| Computations |   2 | 10.6 | 2 million core hours |

#+begin_src R :results output :session *R* :exports both
#borrowed from https://stackoverflow.com/questions/23138388/using-xkcd-package-with-ggplot2-how-do-i-color-a-bar-graph-but-keep-the-man-sol
library(extrafont)
download.file("http://simonsoftware.se/other/xkcd.ttf", dest = "/tmp/xkcd.ttf")
system("mkdir -p ~/.fonts")
system("cp -f /tmp/xkcd.tff -t ~/.fonts")
font_import()
loadfonts()
#+end_src


#+begin_src R :var df=co2_thesis :results output graphics :file (org-babel-temp-file "figure" ".png") :exports both :width 600 :height 400 :session *R*
# #+begin_src R :var df=co2_thesis :results output :session *R* :exports both
library(xkcd)
library(extrafont)
library(dplyr)
library(ggplot2)
...
target = data.frame(co2=c(2), comment=c("2t yearly target"))

width=0.3
plot = df %>%
    ggplot() +
    xkcdrect(aes(xmin=idx-width, xmax=idx+width, ymin=0, ymax=co2, fill=source), data=df) +
    ylab("Number of tonnes of CO2") +
   # xkcdline(aes(y=co2, x=0, yend=co2, xend=2), target, xjitteramount = 0.12) +
    scale_fill_manual(values=c("#2196f3", "#f37e21")) +
    xkcdaxis(c(0.5,2.5), c(0,10)) +
    coord_flip() +
    geom_text(aes(x=idx+width/2, y=1, label=source), hjust=0, size=12, family="xkcd") +
    geom_text(aes(x=idx-width/2, y=1, label=comment), hjust=0, size=6, family="xkcd") +
    theme(legend.position="none") +
...
plot = plot +
    geom_hline(aes(yintercept=co2), target, linetype="dashed") +
    geom_label(aes(x=2.5, y=co2, label=comment), target, size=6, hjust=0.01, family="xkcd")

ggsave(filename='img/slides/co2.pdf', plot=plot, width=5, height=5)
plot
#+end_src

#+RESULTS:
[[file:/tmp/babel-PPrGrA/figuree0DiHz.png]]

#+begin_src R :results output :session *R* :exports both
which(df$source=="Flights")-0.5
#+end_src
