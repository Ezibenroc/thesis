...
scatter_plot = base_plot +
    ...
    geom_path(data=calculate_ellipse(mu, sigma, radius, 100), colour=base_col) +
...
plot_top = base_plot +
    ...
    geom_vline(xintercept=c(mu[1]-radius*std[1], mu[1]+radius*std[1]), color=base_col) +
...
plot_right = base_plot +
    ...
    geom_vline(xintercept=c(mu[2]-radius*std[2], mu[2]+radius*std[2]), color=base_col) +
...
#+RESULTS:
[[file:/tmp/babel-LlZHec/figurezDC7lR.png]]
