...
* TODO  [0/3]
- [ ] Make Zenodo repositories for everything relevant and add the links in the
...
- [ ] Remove all the vspace and hspace. Most of them come from the puslished
...
#+begin_src R :results output graphics :file (org-babel-temp-file "figure" ".png") :exports both :width 400 :height 400 :session *R*
library(dplyr)
plot = dgemm %>% {(#filter(cpu>20, cpu<30) %>%
    ggplot(., aes(x=m*n*k, y=duration)) +
    geom_point(data=filter(., m*n*k<=9.9e9), alpha=.1,size=.5) +
    scale_color_brewer(palette="Set1", guide=F)
)}
plot = plot + theme_bw() + ylab('Duration (s)') +
    scale_x_continuous(name = 'M.N.K', breaks = (0:4)*3E9, limits=c(0,1.2e10)) +
...
plot = plot + geom_smooth(color="#2196F3", method='lm', se=F, fullrange=T, linetype=4)
...
#+RESULTS:
[[file:/tmp/babel-XPvhqG/figureqoBjxf.png]]
