...
do_plot = function(frame, y_val, y_label) {
    ...
    p1 = basic_plot +
     #   geom_quasirandom(aes(x=exp_kind), alpha=0.5, dodge.width=1) +
        geom_boxplot(aes(x=exp_kind), outlier.alpha=0, alpha=0.3) +
        theme(legend.position='none') +
     #   geom_label_repel(data=avg, aes(x=exp_kind, y=val, label=exp_kind), direction="y", alpha=0.5) +
        ylab(y_label) +
    ...
    plot = p1 + p2 + guide_area() + plot_layout(design=layout, guides = 'collect')
    return(plot)
}
#+end_src
...
plot = read.csv("data/experiment/randomizing_sizes/fixing_K/dgemm_agr_data.csv") %>%
    ...
    mutate(start_time=anytime(start_time)) %>%
    do_plot("avg_gflops", "Average performance (Gflop/s)")
ggsave(filename='img/experiment/randomizing_sizes/fixing_K/average_performance.pdf', plot=plot, width=7, height=3)
plot
#+end_src

#+RESULTS:
[[file:/tmp/babel-Zo95a7/figureTqxoV5.png]]
...
#+RESULTS:
[[file:/tmp/babel-Zo95a7/figurejGE7RQ.png]]
...
ggsave(filename='img/experiment/randomizing_sizes/fixing_K/average_frequency.pdf', plot=plot, width=7, height=3)
plot
#+end_src

#+RESULTS:
[[file:/tmp/babel-Zo95a7/figureze9lte.png]]
...
ggsave(filename='img/experiment/randomizing_sizes/fixing_K/average_power_CPU.pdf', plot=plot, width=7, height=3)
plot
#+end_src

#+RESULTS:
[[file:/tmp/babel-Zo95a7/figureLUn3TH.png]]
...
plot = read.csv("data/experiment/randomizing_sizes/fixing_K/dgemm_agr_monitoring.csv") %>%
    ...
    mutate(start_time=anytime(start_time)) %>%
    do_plot("value", "Average DRAM power consumption (W)")
ggsave(filename='img/experiment/randomizing_sizes/fixing_K/average_power_DRAM.pdf', plot=plot, width=7, height=3)
plot
#+end_src

#+RESULTS:
[[file:/tmp/babel-Zo95a7/figurewCm8r5.png]]
**** Distribution of the regression coefficients
#+begin_src R :results output graphics :file (org-babel-temp-file "figure" ".png") :exports both :width 600 :height 400 :session *R*
library(dplyr)
library(ggplot2)
