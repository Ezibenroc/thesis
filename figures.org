...
plot = read.csv('data/context/microprocessor_chronology.csv') %>%
    ...
    select(-processor, -designer, -word_size, -die_per_module_min, -die_per_module_max) %>%
...
#+RESULTS:
[[file:/tmp/babel-BScjbh/figureiN1gRV.png]]
