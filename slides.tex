...
\begin{frame}{Modeling Commputations}
    ...
    \begin{minipage}[t][.7\textheight][t]{\textwidth}
        \begin{center}
            \only<2-4>{Different color $\Rightarrow$ different host}
            \only<5>{For a particular host}

            \includegraphics<2-4>[width=.7\linewidth]{img/prediction/modeling/kernels/dgemm_heterogeneity_calib.png}
            \includegraphics<5>[width=.7\linewidth]{img/prediction/modeling/kernels/dgemm_model_calib.png}
        \end{center}
        \only<6>{
        \textbf{Take-Away Message}:
        \begin{itemize}
        \item Both \alert{spatial} and \alert{temporal} variability
        \item ``Sophisticated'' linear models are \textbf{excellent predictors} (for every function -- \texttt{dtrsm},
            \texttt{daxpy}, \ldots{})
        \end{itemize}
        }
    \end{minipage}
...
\begin{frame}{Modeling Communications}
    ...
    \begin{minipage}[t][.7\textheight][t]{\textwidth}
        ...
        \only<3>{
            \textbf{Take-Away Message}:
            \begin{itemize}
            \item For small messages, the \textbf{variability can be huge}
            \item \textbf{Piece-wise} \textbf{mixture} of \textbf{linear} regressions
            \end{itemize}
        }
    \end{minipage}
\end{frame}
